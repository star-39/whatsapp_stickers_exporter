...
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.ANDROID_APP_DOWNLOAD_LINK_IN_QUERY;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.ANIMATED_STICKER_PACK;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.AVOID_CACHE;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.IOS_APP_DOWNLOAD_LINK_IN_QUERY;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.LICENSE_AGREEMENT_WEBSITE;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.PRIVACY_POLICY_WEBSITE;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.PUBLISHER_EMAIL;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.PUBLISHER_WEBSITE;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.STICKER_FILE_EMOJI_IN_QUERY;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.STICKER_FILE_NAME_IN_QUERY;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.STICKER_PACK_ICON_IN_QUERY;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.STICKER_PACK_IDENTIFIER_IN_QUERY;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.STICKER_PACK_NAME_IN_QUERY;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.STICKER_PACK_PUBLISHER_IN_QUERY;
import static moe.star39.whatsapp_stickers_exporter.StickerContentProvider.IMAGE_DATA_VERSION;
...
class StickerPackLoader {
    ...
    static ArrayList<StickerPack> fetchStickerPacks(Context context) throws IllegalStateException {
        final Cursor cursor = context.getContentResolver().query(WhatsappStickersExporterPlugin.getContentProviderAuthorityURI(context), null, null, null, null);
        if (cursor == null) {
            throw new IllegalStateException("could not fetch from content provider, " + WhatsappStickersExporterPlugin.getContentProviderAuthority(context));
        }
    ...
    static Uri getStickerListUri(Context context, String identifier) {
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(WhatsappStickersExporterPlugin.getContentProviderAuthority(context)).appendPath(StickerContentProvider.STICKERS).appendPath(identifier).build();
    }

    static Uri getStickerAssetUri(Context context, String identifier, String stickerName) {
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(WhatsappStickersExporterPlugin.getContentProviderAuthority(context)).appendPath(StickerContentProvider.STICKERS_ASSET).appendPath(identifier).appendPath(stickerName).build();
    }
}
